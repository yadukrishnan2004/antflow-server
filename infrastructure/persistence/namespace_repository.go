...
// Migrate creates the namespace table if it does not already exist.
...
func (s *PostgresNamespaceRepository) GetByName(ctx context.Context, name string) (*workflow.Namespace, error) {
	ns := &workflow.Namespace{}
	err := s.db.QueryRowContext(ctx,
		`SELECT id, name, created_at FROM namespace WHERE name = $1`, name,
	).Scan(&ns.ID, &ns.Name, &ns.CreatedAt)
	if err == sql.ErrNoRows {
		return nil, workflow.ErrNotFound
	}
